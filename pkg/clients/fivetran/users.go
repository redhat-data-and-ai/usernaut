...
func (fc *FivetranClient) FetchAllUsers(ctx context.Context) (map[string]*structs.User, map[string]*structs.User, error) {
	...
	if err != nil {
		log.WithField("response", resp.CommonResponse).WithError(err).Error("error fetching list of teams")
		return nil, nil, err
	}
...
func (fc *FivetranClient) InviteUser(ctx context.Context, u *structs.User) (*structs.User, error) {
