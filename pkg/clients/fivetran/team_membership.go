...
func (fc *FivetranClient) FetchTeamMembersByTeamID(
	ctx context.Context,
	teamID string,
	usersIDMap map[string]*structs.User) (map[string]*structs.User, error) {
	...
	for _, item := range resp.Data.Items {
		userDetails := usersIDMap[item.UserId]
		teamMembers[userDetails.Email] = userDetails
	}
	...
	for len(cursor) != 0 {
		resp, err := fc.fivetranClient.NewTeamUserMembershipsList().
			...
			Do(ctx)
		if err != nil {
			log.WithField("response", resp.Code).WithError(err).Error("error fetching list of teams")
			return nil, err
		}
		for _, item := range resp.Data.Items {
			userDetails := usersIDMap[item.UserId]
...
func (fc *FivetranClient) AddUserToTeam(ctx context.Context, teamID, userID string) (teams.TeamUserMembership, error) {
	log := logger.Logger(ctx).WithFields(logrus.Fields{
		"service": "fivetran",
		"userID":  userID,
		"teamID":  teamID,
	...
	if err != nil {
		...
		return teams.TeamUserMembership{}, err
