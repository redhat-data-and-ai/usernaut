...
var _ = Describe("Group Controller", func() {
	...
	Context("When reconciling a resource", func() {
		...
		It("should update user ID in cache when gitlab backend returns a different ID", func() {
			...
			// Manually acquire lock to simulate Reconcile flow, then release it
			// This test verifies the logic inside createUsersInBackendAndCache which expects the lock to be held initially
			// However, since we are calling Reconcile() which handles locking internally, we don't need to manually lock here.
			// The key verification is that the operation succeeds (meaning the unlock/lock sequence inside worked correctly)
