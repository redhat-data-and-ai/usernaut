...
func (r *GroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	uniqueMembers, err := r.fetchUniqueGroupMembers(ctx, groupCR.Spec.GroupName, groupCR.Namespace, visitedGroups, false)
	if err != nil {
...
func (r *GroupReconciler) fetchUniqueGroupMembers(ctx context.Context, groupName,
	namespace string, visitedGroups map[string]struct{}, isSubGroup bool) ([]string, error) {

	r.log.WithField("group", groupName).Info("fetching unique group members")

	// Handle cyclic dependencies
	if _, ok := visitedGroups[groupName]; ok {
	...
	if isSubGroup {
		if len(groupCR.Status.ReconciledUsers) == 0 {
			r.log.WithField("group", groupName).Warn("no reconciled users found in the group")
			return []string{}, errors.New("no reconciled users found in the group")
		}
		members = append(members, groupCR.Status.ReconciledUsers...)
	} else {
		members = append(members, groupCR.Spec.Members.Users...)
	}

	for _, subGroup := range groupCR.Spec.Members.Groups {
		subMembers, err := r.fetchUniqueGroupMembers(ctx, subGroup, namespace, visitedGroups, true)
		if err != nil {
			return nil, err
		}
	...
	// deduplicate members
	members = r.deduplicateMembers(members)

	r.log.WithField("group", groupName).WithField("members", len(members)).Info("fetched unique group members")
	return members, nil
}
...
func (r *GroupReconciler) setOwnerReference(ctx context.Context, groupCR *usernautdevv1alpha1.Group) error {
	for _, parentGroup := range groupCR.Spec.Members.Groups {
		parentGroupCR := &usernautdevv1alpha1.Group{}
		if err := r.Client.Get(ctx,
			client.ObjectKey{Namespace: groupCR.Namespace, Name: parentGroup}, parentGroupCR); err != nil {
		...
		groupCR.OwnerReferences = append(groupCR.OwnerReferences, ownerRef)
	}
