...
func (r *GroupReconciler) processUsers(ctx context.Context,
	...
	for _, user := range groupUsers {
		...
		var cachedUser usernautdevv1alpha1.CachedUser
...
func (r *GroupReconciler) createUsersInBackendAndCache(ctx context.Context,
	...
	for _, user := range users {
		...
		cached := usernautdevv1alpha1.CachedUser{Groups: make(map[string][]usernautdevv1alpha1.BackendUser)}
		if userInCache, err := r.Cache.Get(ctx, userDetails.GetEmail()); err == nil {
			_ = json.Unmarshal([]byte(userInCache.(string)), &cached)
		}

		backendUser := usernautdevv1alpha1.BackendUser{Name: backendName, Type: backendType, ID: newUser.ID}
