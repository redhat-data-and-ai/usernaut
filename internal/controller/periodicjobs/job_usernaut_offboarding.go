...
func (uoj *UserOffboardingJob) processUsers(ctx context.Context, userKeys []string) processingResult {
	...
	for _, userKey := range userKeys {
		uoj.logger.WithField("userKey", userKey).Info("Processing user")
...
func (uoj *UserOffboardingJob) processUser(ctx context.Context, userKey string) (bool, error) {
	...
	uoj.logger.WithField("userKey", userKey).Info("User is active in LDAP, no offboarding needed")
	return false, nil
}
...
// removed during the offboarding job execution, with userKey and email for each user.
//
// Parameters:
...
func (uoj *UserOffboardingJob) logOffboardedUsersSummary(offboardedUsers []string) {
	...
	// Create a slice of maps for structured logging
	removedUsers := make([]map[string]string, 0, len(offboardedUsers))
	for _, user := range offboardedUsers {
		removedUsers = append(removedUsers, map[string]string{
			"userKey": user,
		})
	}
